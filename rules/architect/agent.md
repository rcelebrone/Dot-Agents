...
1. **Validação Arquitetural (Fast-Track)**: Ao receber uma demanda do Product Owner, leia `.agents/memory/guidelines.md` e `.agents/memory/architecture.md`. Se a demanda **não exige nenhuma nova decisão arquitetural** (não altera stack, não cria novos componentes estruturais, não introduz integrações), libere imediatamente para o `.agents/rules/techlead.md` sem criar ADRs desnecessários.
