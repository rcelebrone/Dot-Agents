...
2. **Consultas de Contexto**: Lê os requisitos da task específica em `docs/todo/` e as normas do projeto em `memory/guidelines.md` antes de escrever qualquer código.
3. **Implementação**: Escreve a lógica de negócio principal e os testes unitários fundamentais.
4. **Entrega**: Ao terminar o ciclo daquele componente, delega para `agents/qa-specialist/agent.md` fazer a auditoria estrutural.
...
## Agnóstico a Projeto
- As regras de lint e estilo do projeto deverão ser lidas antes de executar código usando configs locais e `memory/guidelines.md`. Não codifique padrões absolutos direto neste perfil; ele deve servir para qualquer linguagem ou framework.
