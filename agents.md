...
- 💻 **[Dev Team](agents/dev-team/agent.md)**: Construção contínua (Clean Code, TDD).
...
1. O **Product Owner** refina a necessidade de negócio, definindo Critérios de Aceite (DoD). Ele pode utilizar `skills/sdlc/feature-flow/SKILL.md`. (Se a task enviada pelo usuário humano já possuir requisitos redigidos no formato adequado, o PO analisa e se valida repassa de imediato sem recriar etapas).
2. O PO delega para o **Tech Lead** especificando o "O Que".
3. O **Tech Lead** traduz essas necessidades em especificações técnicas (criando tarefas isoladas em `docs/tasks/` baseadas no template `docs/templates/task.md`) e aciona o **Dev Team**.
...
`Usuário ➔ Tech Lead`
1. O **Tech Lead** atua na frente de incidentes executando `skills/quality/triage/SKILL.md` para isolar e entender o problema.
...
3. Se for correção técnica, o Tech Lead utiliza `skills/sdlc/feature-flow/SKILL.md` para criar a demanda de correção em `docs/tasks/` baseada no template `docs/templates/bug.md` e delega ao **Dev Team**.
...
2. Atualiza as decisões arquiteturais da aplicação e as diretrizes não funcionais no `rules/guidelines.md`.
...
- **Regras de Domínio:** Ficam centralizadas de maneira global em `rules/business.md`, e seus desdobramentos atômicos ficam persistidos nos arquivos de tarefas da pasta `docs/tasks/` (via Spec Kit).
- **Diretrizes e Padrões (NFRs):** Registrados em `rules/guidelines.md` (e possíveis documentações correlatas em docs). Todos os agentes leem o `rules/guidelines.md` para adequação antes de codificar. **Nota fundamental**: Diferente de todo o framework (que independe de contextos), as decisões mantidas em `rules/business.md`, `rules/architecture.md` e `rules/guidelines.md` NÃO SÃO agnósticas. Eles representam a memória técnica e de negócios estrita do seu repositório. Esses arquivos começam em branco para os novos projetos e a squad tem a responsabilidade de alimentá-los agressivamente com as diretrizes e escolhas conforme avançam.
