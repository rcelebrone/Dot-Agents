...
3. **Tasking**: O Tech Lead gera arquivos técnicos sequenciais `001-nome.md` em `docs/todo/` baseados no documento da feature. O arquivo gerado DEVE refletir a estrutura definida no Spec Kit (`docs/templates/task.md` ou `docs/templates/bug.md`).
4. **Scaffolding**: Gera boilerplate (Entities, Controllers, Services) seguindo o padrão injetado no `memory/architecture.md`.
