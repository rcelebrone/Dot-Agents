...
> **Nota de Design:** Esta é a única skill sem agente dono por definição. Ela existe antes da squad estar configurada — é o que cria as condições para que os agentes operem. É invocada diretamente pelo usuário ou pelo workflow `bootstrap`, nunca por um agente.
...
Antes de preencher os arquivos da pasta `memory/`, a IA executora deve respeitar RIGOROSAMENTE as seguintes fronteiras de responsabilidade para separar regras funcionais, regras não-funcionais e regras de código, armazenando cada aprendizado no local correto:

1. **`memory/business.md` (O Que e Para Quem - Requisitos Funcionais)**:
...
2. **`memory/architecture.md` (Como o sistema opera estruturalmente - Requisitos Não Funcionais)**:
...
3. **`memory/guidelines.md` (Como codificar localmente - Regras de Sintaxe, Código e Restrições)**:
...
2. **Definição de Personalidade (Humor da Squad)**:
   ...
   - Após a resposta, registre a personalidade escolhida no arquivo `memory/guidelines.md` na seção `# 🎭 Personalidade e Tom de Voz`.
...
3. **Memory Init (Mapeamento de Contexto Inicial)**:
   - Escreve e preenche os tópicos no arquivo `memory/business.md` extraindo as regras de negócio primárias e requisitos funcionais detectados nesta primeira varredura do projeto em que foi instalado.
   - Escreve no arquivo `memory/architecture.md` mapeando rigorosamente os requisitos não funcionais, design do sistema, infraestrutura e arquitetura macro de componentes.
   - Escreve no arquivo `memory/guidelines.md` listando todas as tipagens, restrições técnicas, boas práticas e padrões de desenvolvimento base e sintaxe analisadas na pasta inicial do projeto (se já houver código). Lembre-se de adicionar também a regra do Tom/Humor de comunicação nesta etapa.
