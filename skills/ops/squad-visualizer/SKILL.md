...
1. **Verificação de Instalação e Leitura de Registry:**
   ...
   - Consulta o arquivo principal `.agents/workflows/orchestrator.md` e a pasta `.agents/agents/` (incluindo os respectivos arquivos `.md` dos agentes como `product-owner.md`, `techlead.md`, `architect.md`, `developer.md`, `qa-specialist.md`, `security.md`, `ops.md`) para confirmar quais papéis e agentes estão ativos e montar a visão correta do ecossistema.

2. **Reconstrução Dinâmica da Interface (Dashboard Vivo):**
   - O arquivo `dashboard.html` NUNCA deve ser considerado estático. Antes de exibí-lo, o agente atuante DEVE OBRIGATORIAMENTE varrer os arquivos atuais da fundação (`.agents/workflows/orchestrator.md`, os manifestos individuais em `.agents/agents/` e a pasta de `.agents/skills/`).
